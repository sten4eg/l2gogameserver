...
func Handler(client interfaces.ClientInterface, gs GameServerInterface, db *sql.DB) {
	...
	for {
		...
		case clientStates.Authed:
			...
			case 0x0c:
				clientpackets.CharacterCreate(client, data, db)
			case 0x0d:
				clientpackets.CharacterDelete(client, data, db)
			case 0x12:
			...
			case 0xd0:
				if len(data) >= 2 {
					...
					case 0x36:
						clientpackets.RequestGoToLobby(client, db)
					}
				}
			}
		...
		case clientStates.InGame:
			...
			case 0x1c: //tradeDone
				clientpackets.TradeDone(data, client, db)
			case 0x17:
				clientpackets.DropItem(client, data, db)
			//client.Send(pkg)
			//
			...
			case 0x23: //todo пересмотреть
				clientpackets.BypassToServer(data, client, db)
			case 0x19:
				clientpackets.UseItem(character, data, db)
			case 0x31:
			...
			case 0x3f:
				clientpackets.RequestShortCutDel(data, client, db)
			case 0x57:
				clientpackets.RequestRestart(client, db)
				//gameserver.CharOffline(client)
			case 0x60:
				clientpackets.DestroyItem(data, client, db)
			case 0xc1:
			...
			case 0x1f:
				pkg := clientpackets.Action(data, client, broadcast.Checkaem, db)
				if pkg != nil {
					broadcast.Checkaem(client, *pkg)
				}
			case 0x48:
			...
			case 0xce:
				clientpackets.RequestDeleteMacro(client, data, db)
			case 0x56:
			...
			case 0x83:
				clientpackets.RequestPrivateStoreBuy(client, data, db)
			case 0x96:
			...
			case 0x9f:
				clientpackets.RequestPrivateStoreSell(client, data, db)
			}
		}
