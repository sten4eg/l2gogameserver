...
func ItemList(clientI interfaces.ReciverAndSender) []byte {
	...
	for i := range myItems {
		e := &myItems[i]
		buffer.WriteD(e.ObjectId)           // уникальный object_id из бд
		...
		buffer.WriteH(int16(e.ItemType2))   // Item tType 2 : 00-weapon, 01-shield/armor, 02-ring/earring/necklace, 03-questitem, 04-adena, 05-item
		...
		buffer.WriteH(int16(e.GetAttackElementType())) // Тип Аттрибута атаки
