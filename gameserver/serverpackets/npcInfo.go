...
func NpcInfo(npc interfaces.Npcer) []byte {
	...
	buffer.WriteD(int32(npc.GetBaseMagicAttack())) // _mAtkSpd
	buffer.WriteD(int32(npc.GetBaseAttackSpeed())) // _pAtkSpd
	buffer.WriteD(int32(npc.GetRunSpd()))          // _runSpd
	buffer.WriteD(int32(npc.GetWalkSpd()))         // _WalkSpd
	buffer.WriteD(int32(npc.GetRunSpd()))          // _sWimRunSpd (можно сделать отдельный метод при необходимости)
	buffer.WriteD(int32(npc.GetWalkSpd()))         // _sWimWalkSpd
	buffer.WriteD(0)                               // _flyRunSpd
	buffer.WriteD(0)                               // _flyWalkSpd
	buffer.WriteD(0)                               // _flyRunSpd
	buffer.WriteD(0)                               // _flyWalkSpd
	buffer.WriteF(1)                               // _moveMultiplier
